...
<div class="g3 mb22">
  ...
  <div class="bgy-card" onclick="openBgyDetail(<?= $b['id'] ?>)">
    ...
    <div style="display:flex;gap:6px;margin-top:12px;padding-top:10px;border-top:1px solid var(--surface-2)">
      <button class="act-btn btn-xs" onclick="event.stopPropagation();openEditBgy(<?= $b['id'] ?>, '<?= htmlspecialchars(addslashes($b['name'])) ?>', '<?= htmlspecialchars(addslashes($b['municipality'])) ?>', '<?= htmlspecialchars(addslashes($b['captain_name'] ?? '')) ?>', '<?= htmlspecialchars(addslashes($b['contact_no'] ?? '')) ?>', <?= $b['is_active'] ?>)">Edit</button>
...
<div class="modal-overlay" id="modal-edit-bgy">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="form-group"><label>Barangay Name</label><input type="text" id="edit-bgy-name"></div>
      <div class="form-row">
        <div class="form-group"><label>Municipality</label><input type="text" id="edit-bgy-muni"></div>
        <div class="form-group"><label>Captain</label><input type="text" id="edit-bgy-captain"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Contact</label><input type="text" id="edit-bgy-contact"></div>
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-primary" onclick="submitEditBgy()">Save</button>
    </div>
  </div>
</div>
...
function openEditBgy(id, name, muni, captain, contact, active) {
...
function submitEditBgy() {
  const data = { action:'edit', id: document.getElementById('edit-bgy-id').value, name: document.getElementById('edit-bgy-name').value.trim(), municipality: document.getElementById('edit-bgy-muni').value.trim(), captain_name: document.getElementById('edit-bgy-captain').value.trim(), contact_no: document.getElementById('edit-bgy-contact').value.trim(), is_active: document.getElementById('edit-bgy-active').value };
  loading(true);
  fetch('ajax/barangay_action.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(data) })
